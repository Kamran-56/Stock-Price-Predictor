# ============================================================
# Apple Stock Price Predictor - Streamlit GUI
# ============================================================
...
# We use @st.cache_resource so these load only once
...
def predict_tomorrow(headline, df, model, feature_scaler, target_scaler, feature_columns):
    ...
    # Step 3: Replace the news sentiment features in the last row
    # with values from the user's headline
    # This simulates "today's news" being the user's input
    if 'avg_sentiment' in last_60.columns:
        last_60.iloc[-1, last_60.columns.get_loc('avg_sentiment')] = sentiment_score

    if 'max_sentiment' in last_60.columns:
        last_60.iloc[-1, last_60.columns.get_loc('max_sentiment')] = max(
            sentiment_score,
            last_60.iloc[-1][last_60.columns.get_loc('max_sentiment')]
        )

    if 'min_sentiment' in last_60.columns:
        last_60.iloc[-1, last_60.columns.get_loc('min_sentiment')] = min(
            sentiment_score,
            last_60.iloc[-1][last_60.columns.get_loc('min_sentiment')]
        )

    if 'high_impact_count' in last_60.columns and impact_tier == 'HIGH':
        last_60.iloc[-1, last_60.columns.get_loc('high_impact_count')] += 1

    if 'medium_impact_count' in last_60.columns and impact_tier == 'MEDIUM':
        last_60.iloc[-1, last_60.columns.get_loc('medium_impact_count')] += 1

    if 'negative_count' in last_60.columns and sentiment_score < 0:
        last_60.iloc[-1, last_60.columns.get_loc('negative_count')] += 1

    if 'news_count' in last_60.columns:
        last_60.iloc[-1, last_60.columns.get_loc('news_count')] += 1

    # Step 4: Scale the 60-day window using the feature scaler
    ...
    # Step 7: Convert back to real dollar price
...
st.markdown('---')
st.markdown(
    '**Disclaimer:** This is a BTech academic project. '
    'Predictions are for educational purposes only and should '
    'NOT be used for real investment decisions.'
)
